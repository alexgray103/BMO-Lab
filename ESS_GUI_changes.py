...
import serial
#create new folders to hold data and settings
import os
#saving data to csv
import pandas as pd
import csv
...
class Main_GUI:
    def __init__(self, master):
        ...
        try:
            ...
            messagebox.showinfo("Directory", "All Files will be stored in folder Spectrometer")
        # setup Serial port
        port = "/dev/ttyACM0"
        port2 = "/dev/ttyACM1"
        try:
        ...
        # if no settings file exists create one into designalted folder
        # write default settings to new csv if none there
        global settings_file
        settings_file = '/home/pi/Desktop/Spectrometer/settings.csv'
        try:
        ...
        #except:
            #messagebox.showerror("Error", "No Settings File Found, Create settings File in designated folder")
        ...
        #either create new file in the designated path 
        file_acquire = '/home/pi/Desktop/Spectrometer/acquire.csv'
        try:
            open_acquire_file = open(file_acquire, 'x')
        except:
            open_acquire_file = open(file_acquire, 'a')
        
        # intialize arduino with one psuedo read
        ser.write(b"read\n")
        data = ser.readline()
        ...
        def acquire_save():
            ...
            ser.write(b"read\n")
            try:
                #read data and save to one large array then print to csv
                data = ser.readline()
                data_array = np.array([int(p) for p in data.split(b",")])
                dummy = pd.read_csv(filename, header = None) #previous data to be concatenated
                data_saved = np.column_stack([dummy, data_array]) #most recent data to append to array 
                Data_panda = pd.DataFrame(data_saved)
                ...
                messagebox.showerror("Error", "No File Path Entered, Please Open New experiment with File Name")
        ...
        def clear():
            ...
            try:
                ...
                fig.canvas.draw()
        def ratio_view():
            np.true_divide(x, 4) # use division with the given reference spectra
            plt.clf()
            try:
                pixel = range(0,288,1)
                data = pd.read_csv(filename, header = None)
                reference = pd.read_csv(filename_ref, header = None)
                for col in range(1, len(data.columns),1):
                    plt.plot(pixel,data.loc[:,col])
                fig.canvas.draw()
            except:
        ...
        canvas.get_tk_widget().pack()
        sleep(1)
        #fig.add_subplot(111).plot(pixel,ran)
