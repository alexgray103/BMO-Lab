...
class functions:
    ...
    def plotting(self, data):
        ...
        if self.ratio_view_handler:
            ...
            if self.add_remove_top.data_headers is not None:
                data_sel = pd.read(self.save_file, header = 0)
    ...
    def plot_selected(self):
        ...
        try:
            ...
            plt.legend(self.add_remove_top.data_headers, loc = "upper right", prop ={'size': 6}, bbox_to_anchor = (1.18,0.5))
    ...
    def acquire(self, save):
        ...
        if save:
            if self.save_file == None:
                messagebox.showerror('Error', 'No Experiment File Found, create or open File to save spectra')
            else:
    ...
    def sequence(self, save):
        ...
        for burst in range(0,burst_number):
            ...
            for i in range(0,number_measurements_burst):
                ...
                plt.subplots_adjust(bottom = 0.14, right = 0.80)
                plt.legend(loc = "center right", prop = {'size': 6}, bbox_to_anchor = (1.285,0.5))
                self.fig.canvas.draw()
